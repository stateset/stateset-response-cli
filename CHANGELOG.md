...
- Made interactive help and completion more discoverable with category-aware `/help`, fuzzy help lookups, and clearer integrations command coverage.
- Added `response ask` for single-shot prompts with session support, attachments, and `--stdin` input so the CLI works better in scripts and shell pipelines.
- Documented the new one-shot workflow in the README and getting-started guide.

### Runtime & Packaging

- Added runtime relaunch wrappers so shipped binaries can recover from an older shell `node` by finding and re-execing a compatible Node 18+ runtime.
- Hardened the packaged launcher path and extended bin smoke coverage around the public entrypoints.

### Tests

- Added focused regression coverage for one-shot input parsing and Node runtime launcher behavior under piped execution.
