...
vi.mock('../config.js', () => ({
  ...
  getConfiguredModel: vi.fn(() => 'claude-sonnet-4-20250514'),
...
describe('Orchestrator', () => {
  ...
  it('resolves configured model using strict validation', async () => {
    const orchestrator = new Orchestrator({ model: 'claude-sonnet-4-20250514' });
    vi.spyOn(
      orchestrator as { startSlack: (_model: unknown) => Promise<StartResult> },
      'startSlack',
    ...
    expect(resolveModelOrThrow).toHaveBeenCalledWith('claude-sonnet-4-20250514', 'valid');
    await orchestrator.stop();
  });
