...
export function registerFunctionTools(server: McpServer, client: GraphQLClient, orgId: string) {
  ...
  server.tool(
    ...
    'Create a new function (API call definition)',
    {
      ...
      function_type: z.string().max(50).optional().describe('Type of function (default: "api_call")'),
      ...
      timeout: z.number().int().min(1000).max(300000).optional().describe('Timeout in milliseconds (1s-5min)'),
  ...
  server.tool(
    ...
    'Bulk import functions (upserts on primary key conflict)',
    {
      ...
      })).max(100).describe('Array of functions to import (max 100)'),
    },
    async ({ functions }) => {
