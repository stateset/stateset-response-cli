...
export function registerFunctionTools(server: McpServer, client: GraphQLClient, orgId: string) {
  ...
  server.tool(
    ...
    {
      limit: z.number().optional().describe('Max number of functions to return (default 100)'),
    ...
    async ({ limit, offset }) => {
      ...
      return { content: [{ type: 'text' as const, text: JSON.stringify(data.functions, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    'Create a new function (API call definition)',
    {
      ...
      agent_id: z.string().describe('UUID of the agent this function belongs to'),
      endpoint: z.string().describe('API endpoint URL'),
      method: z.string().describe('HTTP method (GET, POST, PUT, DELETE)'),
      function_type: z.string().optional().describe('Type of function (default: "api_call")'),
      description: z.string().optional().describe('Description'),
      activated: z.boolean().optional().describe('Whether the function is activated'),
      parameters: z.array(z.any()).optional().describe('Function parameters'),
      authentication: z.any().optional().describe('Authentication config (e.g. {type: "bearer"})'),
      headers: z.any().optional().describe('Custom headers'),
      request_transform: z.any().optional().describe('Request body transform'),
      response_handling: z.any().optional().describe('Response handling config'),
      retry_config: z.any().optional().describe('Retry configuration'),
      timeout: z.number().optional().describe('Timeout in milliseconds'),
      rate_limit: z.any().optional().describe('Rate limit config'),
    },
    async (args) => {
      ...
      const fn = {
        ...
        response_handling: args.response_handling || { success_condition: 'status_code == 200', error_message_path: 'error.message', result_mapping: {} },
        retry_config: args.retry_config || { max_attempts: 3, backoff: 'exponential', retry_on: [502, 503, 504] },
        timeout: args.timeout || 30000,
        rate_limit: args.rate_limit || { requests_per_minute: 60 },
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      };
      const data = await executeQuery<{ insert_functions: { returning: unknown[] } }>(client, mutation, { _function: fn });
      return { content: [{ type: 'text' as const, text: JSON.stringify(data.insert_functions.returning[0], null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    {
      id: z.string().describe('UUID of the function to update'),
      function_name: z.string().optional().describe('New name'),
      function_type: z.string().optional().describe('New type'),
      ...
      parameters: z.array(z.any()).optional().describe('Updated parameters'),
      authentication: z.any().optional().describe('Updated auth config'),
      headers: z.any().optional().describe('Updated headers'),
    ...
    async (args) => {
      ...
      const data = await executeQuery<{ update_functions: { returning: unknown[] } }>(client, mutation, { id, org_id: orgId, set: setFields });
      if (!data.update_functions.returning.length) {
        return { content: [{ type: 'text' as const, text: 'Function not found' }], isError: true };
      }
      return { content: [{ type: 'text' as const, text: JSON.stringify(data.update_functions.returning[0], null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    { id: z.string().describe('UUID of the function to delete') },
    async ({ id }) => {
      ...
      const data = await executeQuery<{ delete_functions: { returning: unknown[] } }>(client, mutation, { id, org_id: orgId });
      if (!data.delete_functions.returning.length) {
        return { content: [{ type: 'text' as const, text: 'Function not found' }], isError: true };
      }
      return { content: [{ type: 'text' as const, text: JSON.stringify({ deleted: data.delete_functions.returning[0] }, null, 2) }] };
    }
  );
  ...
  server.tool(
    ...
    'Bulk import functions (upserts on primary key conflict)',
    {
      ...
      })).describe('Array of functions to import'),
    },
    async ({ functions }) => {
      ...
      const prepared = functions.map(f => ({
        ...
        response_handling: { success_condition: 'status_code == 200', error_message_path: 'error.message', result_mapping: {} },
      ...
      const data = await executeQuery<{ insert_functions: { returning: unknown[]; affected_rows: number } }>(client, mutation, { functions: prepared });
      return { content: [{ type: 'text' as const, text: JSON.stringify({ affected_rows: data.insert_functions.affected_rows, functions: data.insert_functions.returning }, null, 2) }] };
    }
  );
}
