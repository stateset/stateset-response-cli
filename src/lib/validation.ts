...
export const limitedString = (maxLength = MAX_STRING_LENGTH) =>
...
export const paginationLimit = z.number().int().min(1).max(1000).optional()
  .describe('Max number of items to return (1-1000, default 100)');

export const paginationOffset = z.number().int().min(0).max(100000).optional()
...
).optional().describe('Additional metadata key-value pairs');
...
const singleCondition = z.object({
...
export const conditionsSchema = z.object({
  any: z.array(singleCondition).max(20).optional()
    .describe('Match if ANY condition is true (OR)'),
  all: z.array(singleCondition).max(20).optional()
    .describe('Match if ALL conditions are true (AND)'),
}).optional().describe('Conditions object with "any" or "all" arrays');

// ============================================================================
// Actions Schema (for rules/skills)
// ============================================================================

const actionParams = z.record(
  z.string().max(100),
  z.union([
    z.string().max(MAX_STRING_LENGTH),
    z.number(),
...
const singleAction = z.object({
  type: z.string().max(50).describe('Action type (e.g., "send_message", "update_field", "call_function")'),
...
export const actionsSchema = z.array(singleAction).max(20).optional()
...
export const parametersSchema = z.array(functionParameter).max(50).optional()
...
export const authenticationSchema = z.object({
  ...
  }).optional().describe('OAuth2 configuration'),
...
export const headersSchema = z.record(
  z.string().max(100),
  z.string().max(2000)
).optional().describe('Custom HTTP headers');

/** Request body transform */
export const requestTransformSchema = z.object({
  body: z.record(z.string().max(100), z.unknown()).optional()
...
}).optional().describe('Request body transformation');

/** Response handling configuration */
export const responseHandlingSchema = z.object({
  success_condition: z.string().max(500).optional()
    .describe('Expression to determine success (e.g., "status_code == 200")'),
  error_message_path: z.string().max(200).optional()
    .describe('JSON path to error message in response'),
  result_mapping: z.record(z.string().max(100), z.string().max(500)).optional()
    .describe('Map response fields to output'),
}).optional().describe('Response handling configuration');

/** Retry configuration */
export const retryConfigSchema = z.object({
  max_attempts: z.number().int().min(0).max(10).optional()
    .describe('Maximum retry attempts (0-10)'),
  backoff: z.enum(['none', 'linear', 'exponential']).optional()
    .describe('Backoff strategy'),
  retry_on: z.array(z.number().int().min(400).max(599)).max(10).optional()
    .describe('HTTP status codes to retry on'),
  retry_delay_ms: z.number().int().min(100).max(60000).optional()
...
export const rateLimitSchema = z.object({
  ...
  concurrent_requests: z.number().int().min(1).max(100).optional()
...
export const attributeValueSchema = z.union([
  ...
  z.boolean(),
  z.null(),
]).optional().describe('Attribute value');
...
export const ticketContentSchema = z.object({
  customer_message: z.string().max(MAX_STRING_LENGTH).optional()
    .describe('Original customer message'),
  sentiment: z.string().max(50).optional()
    .describe('Detected sentiment'),
...
}).optional().describe('Ticket content object');

/** Response content for examples */
export const responseContentSchema = z.object({
  message: z.string().max(MAX_STRING_LENGTH).optional()
    .describe('Agent response message'),
  tone: z.string().max(50).optional()
    .describe('Response tone'),
  actions_taken: z.array(z.string().max(200)).max(20).optional()
    .describe('Actions taken by agent'),
  follow_up_required: z.boolean().optional()
    .describe('Whether follow-up is needed'),
  resolution_status: z.string().max(50).optional()
    .describe('Resolution status'),
}).optional().describe('Response content object');
...
export const bulkIdsSchema = z.array(
  z.string().uuid()
